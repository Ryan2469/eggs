 ...
 * @version 0.1
 */
class xeant extends WidgetHandler
{
	/**
	...
	function proc($args)
	{	
		if(!$args->content_type) $content_type= "document"; 
		// Targets to sort
		...
		if(!$args->thumbnail_width) $args->thumbnail_width = 300;
		// Vertical size of thumbnails
		if(!$args->thumbnail_height) $args->thumbnail_height = 300;
		// Viewing options
		$args->option_view_arr = 'title,regdate,nickname,thumbnail,content';
		// markup options
	...
	/**
	 * @brief Get a list of comments and return contentXeant
	 */
	function _getCommentItems($args)
	{
		...
		foreach($output as $key => $oComment)
		{
			...
			$content_item = new contentXeant($browser_title);
	...
	function _getDocumentItems($args)
	{
		...
		if(is_array($output->data) && count($output->data))
		{
			...
			foreach($document_srls as $i => $document_srl)
			{
				...
				$content_item = new contentXeant($args->module_srls_info[$module_srl]->browser_title);
	...
	function _compile($args,$content_items)
	{
		...
		$widget_info->show_browser_title = 'Y';
		$widget_info->show_category = 'Y';
		$widget_info->show_comment_count = 'Y';
		$widget_info->show_trackback_count = $args->show_trackback_count;
		$widget_info->show_icon = 'Y';
		...
		if($args->list_type == 'gallery'){
			return $oTemplate->compile($tpl_path, "gallery");
		}else{
...
class contentXeant extends BaseObject
{
